...
[our demo for large sample](#score-to-risk) for detail.

## Demo 3: data with missing values {#demo3}

In Demo #3, we demonstrate the use of AutoScore on a dataset with missing values in two variables (i.e., 
`r paste(names(sample_data)[1:2], sep = " and ")`). 

```{r, include=FALSE}
data("sample_data_survival")
sample_data_survival_missing <- AutoScore:::induce_informative_missing(
  sample_data_survival, vars_to_induce = names(sample_data_survival)[1:2], 
...
`Unknown`. The following steps are the **same** as those in [Demo 1](#demo1).
