...
## Demo 1: Development with large sample {#demo1}
...
::: callout-important
- Before proceeding, follow the steps in [Chapter 4](03-data_processing.qmd#top)
...
- A lookup table maps scores to predicted risks, visualised using an interactive
figure below.
...
average predicted risk within each score interval, which can be used as lookup
table to predict risk for a new subject.
...
## Demo 2: Development with small sample {#demo2}
...
<!-- When the proportion of missing entries in some variables is large enough for -->

<!-- missing to be modelled as a separate category (which applies to both continuous -->

<!-- and categorical predictors), users may directly apply AutoScore-Ordinal to the -->

<!-- dataset with missing.  -->

- For demonstration, we manually induce missing to two variables in the sample
data (20% in `Lab_A` and 60% in `Vital_A`.), assuming values close to normal
range (i.e., median value in the complete data) are more likely to be missing.
...
- As demonstrated below, such missingness pattern has little
impact on variable ranking and cutoff values. 
