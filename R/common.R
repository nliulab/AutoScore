...
#' @title AutoScore Function: Print plotted variable importance
...
plot_importance <- function(ranking){
  ...
  df = as.data.frame(ranking, row.names = names(ranking))
  df$var = rownames(df)
  df$rank = c(1:nrow(df))
  colnames(df) = c("Imp", "Var", "Rank")
  df <- df %>% arrange(desc(Rank))
  df$Index = c(1:nrow(df))
  yaxis <- df$Var
  
  p <- ggplot(data = df, mapping = aes_string(x = "Index", y = "Imp")) +
    geom_bar(stat = "identity",fill = "deepskyblue") +
    coord_cartesian(ylim = c(0.95*min(ranking), 1.05*max(ranking))) +
    labs(x = "", y = "Importance", title = "") + 
    ggtitle("Importance Ranking") +
  ...
  p <- p + coord_flip()
  p <- p + scale_x_continuous(breaks = 1:nrow(df), labels = yaxis)
