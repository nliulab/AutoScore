...
print_score_table<-function(scoring_table,final_variable){
  ...
  for (i in 1:length(final_variable)) {
    ...
    rank_indicator[which(rank_indicator=="")]<-as.numeric(max(rank_indicator))*1.1
...
transform_df_fixed <- function(df, cut_vec) {
  ...
  for (i in 1:(length(df) - 1)) {
    ...
    if(max(vec) >= cut_vec[[j]][length(cut_vec[[j]])] )  cut_vec_new <- c(cut_vec_new, ceiling(max(df[, i])*1.2))
...
#'
#' @description 20000 simulated samples, with the same distribution as the data in the MIMIC-III ICU database. It is used for demostration only in the Guidebook.
"sample_data"
